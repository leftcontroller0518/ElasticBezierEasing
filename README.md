# エラスティックベジェイージング
> [!NOTE]
> このプラグインはharmonyというライブラリを使用させていただいています。

## 機能
- YMM4のイージング選択（AnimationSliderの右クリックメニュー）に「エラスティックベジェ」を追加
- 右クリック→「編集...」で専用のイージング編集ウィンドウ（WPF）を開く
...
## イージング関数について
```
f(t) = t + Amplitude * sin(2π * Oscillation * t) * (1 - t) ^ DampingExponent
```
...
他のプラグイン（IterativeMovementExtension含む）とIDが競合する場合は、
